	...
	for ( var int = 0; int < array.length; int++) {
		do {
			...
			Wirelib.prototype.addLine = function() {
				...
				points = (typeof arguments[0] === "object") ? arguments[0]
						: arguments;
				...
				if (numPoints >= 6) {
					...
					console
							.error("wirelib.addLine: You need to add at least two 3d points (6 numbers) to make a line.");
				}
				return line;
			};
			Wirelib.prototype.addCube = function(x, y, z, w, h, d) {
				this.addLine(x - w / 2, y - h / 2, z - d / 2, x + w / 2, y - h
						/ 2, z - d / 2, x + w / 2, y + h / 2, z - d / 2, x - w
						/ 2, y + h / 2, z - d / 2, x - w / 2, y - h / 2, z - d
						/ 2);
				this.addLine(x - w / 2, y - h / 2, z + d / 2, x + w / 2, y - h
						/ 2, z + d / 2, x + w / 2, y + h / 2, z + d / 2, x - w
						/ 2, y + h / 2, z + d / 2, x - w / 2, y - h / 2, z + d
			...
			Wirelib.prototype.addRect = function(x, y, z, w, h) {
				this.addLine(x - w / 2, y - h / 2, z, x + w / 2, y - h / 2, z,
						x + w / 2, y + h / 2, z, x - w / 2, y + h / 2, z, x - w
								/ 2, y - h / 2, z);
			};
			Wirelib.prototype.addCircle = function(x, y, z, radius, segments) {
				...
				for (i = 0; i < segments; i += 1) {
					...
					points.push(x + Math.cos(a) * radius, y + Math.sin(a)
							* radius, z);
				}
			...
			Wirelib.prototype.rotateX = function(radians) {
				var i, j, p, y1, z1, line, cos = Math.cos(radians), sin = Math
						.sin(radians);
				for (i = 0; i < this.lines.length; i += 1) {
					line = this.lines[i];
			...
			Wirelib.prototype.rotateY = function(radians) {
				var i, j, p, x1, z1, line, cos = Math.cos(radians), sin = Math
						.sin(radians);
				for (i = 0; i < this.lines.length; i += 1) {
					line = this.lines[i];
					for (j = 0; j < line.points.length; j += 1) {
			...
			Wirelib.prototype.rotateZ = function(radians) {
				var i, j, p, x1, y1, line, cos = Math.cos(radians), sin = Math
						.sin(radians);
				for (i = 0; i < this.lines.length; i += 1) {
					line = this.lines[i];
					for (j = 0; j < line.points.length; j += 1) {
